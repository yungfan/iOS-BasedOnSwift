...
extension ViewController : UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            self.detailContent?.remove(at: indexPath.row)
            ...
            //1.增加一条数据
            self.content?.insert("iPhone 1", at: indexPath.row)
            self.detailContent?.insert("iPhone 1 - iPhone OS", at: indexPath.row)
            //2.增加一行
    ...
    func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        ...
        let contentText = self.content?[sourceIndexPath.row]
        
        self.content?.remove(at: sourceIndexPath.row)
        
        self.content?.insert(contentText!, at: destinationIndexPath.row)
        
        
        let detailContentText = self.detailContent?[sourceIndexPath.row]
        
        self.detailContent?.remove(at: sourceIndexPath.row)
        
        self.detailContent?.insert(detailContentText!, at: destinationIndexPath.row)
