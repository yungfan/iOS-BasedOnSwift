...
typealias HTTPSessionDataTaskCompletionBlock = (_ response: URLResponse?, _ responseObject: Any?, _ error: Error?) -> Void
...
class NetworkToolsDelegate: NSObject, URLSessionDataDelegate {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        let responseStr = String(data: buffer, encoding: String.Encoding.utf8)
